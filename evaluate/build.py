...
def build_image(docker_name, dockerfile_path, context_path, error_log_file):
    ...
    temp_dir = os.path.dirname(error_log_file)  # Get log file directory
    ...
    try:
        ...
        process = subprocess.Popen(
            [
                "docker", "build",
                "-t", docker_name,
                "-f", temp_dockerfile_path,  # Absolute path to Dockerfile
                "."
            ],
        ...
        while True:
            ...
            rlist, _, _ = select.select([process.stdout], [], [], 0.1)  # 0.1 second timeout
...
def process_dockerfiles(project_path, file_folder, evaluate_result_path, context_folder, image_name):
    ...
    for index, dockerfile_name in enumerate(tqdm(dockerfiles)):
        try:
            ...
            time.sleep(3)  # Interval to prevent resource conflicts
    ...
    print("\n[Status] All image builds completed, starting to clean dangling images...")
...
if __name__ == "__main__":
    ...
    # Define positional arguments (parsed in order)
    parser.add_argument("file_folder", help="Dockerfile storage directory (e.g., dataset_fast/star1000+_dockerfile/)")
    parser.add_argument("evaluate_result_path", help="Evaluation result directory (e.g., evaluate_result/ctf/dockerfile/)")
    parser.add_argument("context_folder", help="Build context directory (e.g., dataset_fast/star1000+_context/)")
    parser.add_argument("image_name", help="Build image name (e.g., star or ctf)")
